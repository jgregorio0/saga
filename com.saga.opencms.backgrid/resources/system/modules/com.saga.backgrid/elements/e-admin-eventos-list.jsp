...
|%(link.weak:/system/modules/com.saga.sagasuite.core.script/resources/jquery-ui/1.10.3/js/jquery-ui-1.10.3.Datepicker.js:f31bfd97-bf30-11e3-9278-f18cf451b707)
|%(link.weak:/system/modules/com.saga.sagasuite.core.script/resources/jquery-ui/1.10.3/js/jquery.ui.datepicker-es.js:ade89f28-7f63-11e3-9005-f18cf451b707)
...
<cms:bundle basename="com.saga.ausape.zonaprivada.messages">
	<div>
		...
		<div id="filters" class="row">
			<div id="filter-txt" class="col-xs-4"></div>
			<div id="filter-date-start" class="col-xs-4"></div>
			<div id="filter-date-end" class="col-xs-4"></div>
		</div>
		<div id="events"></div>
	</div>
</cms:bundle>
...
<script>
	$(function(){
		...
		var $gridDiv = $("#events");
		var $divFilterTxt = $("#filter-txt");
		var $divFilterDateStart = $("#filter-date-start");
		var $divFilterDateEnd = $("#filter-date-end");
		...
		var Events = AuPageableCollection.extend({
			model: Event,
			url: "<cms:link>/system/modules/com.saga.ausape.zonaprivada/elements/e-admin-eventos-controller.jsp</cms:link>",
			mode: "server",
			state: {
				pageSize: 10,
				order: 1
		...
		// FILTRO DE TEXTO
		var serverSideFilterTxt = new Backgrid.Extension.ServerSideFilter({
			collection: events,
			name: "ftxt",
			placeholder: "Busque evento",
			fields: ['xmltitle_es_s'],
			template: function (data) {
				return '<input type="search" ' + (data.placeholder ? 'placeholder="' + data.placeholder + '"' : '') + ' name="' + data.name + '" ' + (data.value ? 'value="' + data.value + '"' : '') + '/><a data-backgrid-action="clear" href="#"><span class="fa fa-times">&nbsp;</span></a>';
			}
		});

		// FILTRO DE FECHA INICIO
		var serverSideFilterDateStart = new AuDateFilter({
			collection: events,
			name: "fdatestart",
			placeholder: "Desde",
			fields: ['xmldate_es_dt']
		});

		// FILTRO DE FECHA FIN
		var serverSideFilterDateEnd = new AuDateFilter({
			collection: events,
			name: "fdateend",
			placeholder: "Hasta",
			fields: ['xmldate_es_dt']
		...
		$divFilterTxt.append(serverSideFilterTxt.render().el);
		$divFilterDateStart.append(serverSideFilterDateStart.render().el);
		$divFilterDateEnd.append(serverSideFilterDateEnd.render().el);
