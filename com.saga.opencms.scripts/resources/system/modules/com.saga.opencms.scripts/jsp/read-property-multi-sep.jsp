...
<%
  ...
  Set totalValues = new HashSet<String>();
...
//        LOG.error("Ejecutamos script ajax=true");
    ...
    String sepRegex = request.getParameter("sepRegex");
...
//                LOG.error("resource: " + resource);
        try {
          ...
          if (!cmsProperty.isNullProperty() && !StringUtils.isBlank(cmsProperty.getValue())) {
            String valueStr = cmsProperty.getValue();
            String[] valuesArray = valueStr.split(sepRegex);
            List<String> valuesList = Arrays.asList(valuesArray);
            results.put(resource.getRootPath(), valuesList);
            totalValues.addAll(valuesList);
...
<div>
  ...
  <c:forEach items="<%=results.entrySet()%>" var="result" varStatus="status">
    ...
    <p>
      ${result.key}
        <c:forEach items="<%=totalValues%>" var="val">
           ;
           <c:if test="${result.value.contains(val)}">
             ${val}
           </c:if>
        </c:forEach>
     </p>
      ...
      <div class="row">
        <div class="col-sm-4">
          <form method="post" id="scriptExecForm">
            ...
            <div>
              <label for="sepRegex">Separado REGEX:</label>
              <input type="text" class="form-control" id="sepRegex" name="sepRegex" value="<c:out value='${param.sepRegex }'/>" placeholder=";">
            </div>
