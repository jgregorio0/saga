...
//        LOG.error("Ejecutamos script ajax=true");
    ...
    String resourcesStr = request.getParameter("resourcesStr");
//    String propName = request.getParameter("propName");
//        LOG.error("parametros rootPath: " + rootPath + " propName: " + propName);
    try {

      List<String> resources = Arrays.asList(resourcesStr.split("\\|"));
      for (int i = 0; i < resources.size(); i++) {
        String resRootPath = resources.get(i);
        try {
          CmsResource resource = cmso.readResource(resRootPath);
          CmsProperty cmsProperty = cmso.readPropertyObject(resource, "cache", false);
          String propValue = cmsProperty.getValue();
          if (!cmsProperty.isNullProperty() && !StringUtils.isBlank(propValue)) {
            if (propValue.contains("locale")) {
              results.put("CONTENIDA -- " + resRootPath, "contiene el valor locale en la propiedad cache (" + propValue + ")");
            } else {
              String newPropValue = propValue + ";locale";
              try {
                String sitePath = cmso.getSitePath(resource);
                lock(cmso, sitePath);
                cmso.writePropertyObject(resRootPath, new CmsProperty("cache", newPropValue, newPropValue));
                results.put("INCLUIDA -- " + resRootPath, "Se le añade el valor locale a la propiedad" + resRootPath + " (" + newPropValue + ")");
              } catch (Exception e) {
                errors.add("<p><b>ERROR</b> añadiendo el valor locale en el recurso " + resRootPath + "</p>");
                LOG.error("error añadiendo locale " + resRootPath, e);
              }
            }
          } else {
            results.put("NO TIENE CACHE -- " + resRootPath, "El recurso " + resRootPath + " no tiene propiedad cache");
          }
        } catch (Exception e) {
          LOG.error("error añadiendo locale " + resRootPath, e);
        }
      ...
      <div class="row">
        <div class="col-sm-4">
          <form method="post" id="scriptExecForm">
            ...
            <div>
              <label for="resourcesStr">Rutas raíz separadas por tubería (|):</label>
              <input type="text" class="form-control" id="resourcesStr" name="resourcesStr" value="<c:out value='${param.resourcesStr }'/>" placeholder="/system/modules/com.saga.sagasuite.transparencia/elements/e-ubicacion.jsp|/system/modules/com.saga.udl.frontend/elements/e-menu.jsp">
            </div>
