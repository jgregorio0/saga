...
//        loadParamsFromMultiPart(fileItems)
        uploadFolder = getPageContext().getAttribute(CONF_FOLDER);

        String maxSizeKBytesStr = getPageContext().getAttribute(CONF_SIZE);
        maxSizeBytes = decodeFileSize(maxSizeKBytesStr);

        String formatsStr = getPageContext().getAttribute(CONF_FORMAT);
     ...
     * @return
     */
    def loadParamsFromMultiPart(List<FileItem> fileItems) {
    ...
    def loadJsonConf(String path) {
        // Load JSON
        CmsFile jsonFile = cmso.readFile(path)
        String jsonStr = new String(jsonFile.getContents(), encode)
        JSONObject json = new JSONObject(jsonStr)

        // load config
        json.getString(CONF_FOLDER)
        json.getString(CONF_SIZE)
        json.getString(CONF_FORMAT)
    }
/**
     * Validate parameters
     * @return
